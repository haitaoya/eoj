...
public class ContestsController {
	...
	public ModelAndView leaderboardView(
		...
		if ( contest.getContestMode().equals("ACM") ) {
		...
		Map<Long, Map<Long, Integer>> wrongsubmissions = (Map<Long, Map<Long, Integer>>) result.get("wrongsubmissions");
		view.addObject("contestants", contestants);
		view.addObject("submissions", submissions);
		view.addObject("contest", contest);
		view.addObject("wrongsubmissions", wrongsubmissions);
	 ...
	 */
	@RequestMapping(value="/{contestId}/p/{problemId}/{problemIdInContest}", method=RequestMethod.GET)
	public ModelAndView problemView(
			@PathVariable("contestId") long contestId,
			@PathVariable("problemId") long problemId,
			@PathVariable("problemIdInContest") int problemIdInContest,
			HttpServletRequest request, HttpServletResponse response) {
		HttpSession session = request.getSession();
		User currentUser = HttpSessionParser.getCurrentUser(session);
		Contest contest = contestService.getContest(contestId);
		...
		view.addObject("problemIdInContest", problemIdInContest);
	...
	public @ResponseBody Map<String, Object> createSubmissionAction(
		...
		if ( !contest.getEndTime().after(currentTime)) {
			throw new ResourceNotFoundException();
		}
		...
		if ( isSuccessful ) {
			...
			contestService.createContestSubmission(contestId,submissionId);
