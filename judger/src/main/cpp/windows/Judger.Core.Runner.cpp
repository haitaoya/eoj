...
#include "../com_easyoj_jni_hashmap.h"
#include "../com_easyoj_jni_library.h"
#include "../com_easyoj_judger_core_Runner.h"
 ...
 * @param  jInputFilePath  - 执行程序时的输入文件路径(可为NULL)-编译用
 ...
 */
JNIEXPORT jobject JNICALL Java_com_easyoj_judger_core_Runner_getRuntimeResult(
...
void setupStartupInfo(STARTUPINFOW& startupInfo, HANDLE& hInput, HANDLE& hOutput) {
    ...
    startupInfo.dwFlags        |= STARTF_USESTDHANDLES;//|=是按位或之后赋值
 ...
 * @param  startupInfo   - a STARTUPINFO 结构体
...
bool createProcess(const std::wstring& commandLine, const std::wstring& username,
    ...
    LPCWSTR lpDomain            = getConstWideStringPointer(L".");//L+字符串 将ANSI字符串转换成unicode
