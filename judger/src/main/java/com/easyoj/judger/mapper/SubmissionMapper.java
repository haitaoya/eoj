...
import com.easyoj.judger.model.Problem;
...
public interface SubmissionMapper {
	 ...
	 * @return 一个评测记录对象
	 */
	@Select("SELECT * FROM eoj_submissions WHERE submission_id = #{submissionId}")
	@Options(useCache = true)
	@Results({
		@Result(property = "submissionId", column = "submission_id"),
		@Result(property = "problem", column = "problem_id", javaType = Problem.class, one = @One(select="com.easyoj.judger.mapper.ProblemMapper.getProblem")),
		@Result(property = "uid", column = "uid"),
		@Result(property = "language", column = "language_id", javaType=Language.class, one = @One(select="com.easyoj.judger.mapper.LanguageMapper.getLanguageUsingId")),
